    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <div className="space-y-1">
          ...
          {editingName ? (
            <div className="flex gap-1.5">
              <input
                className="flex-1 bg-zinc-800 border border-zinc-600 rounded px-2 py-1 text-xs text-zinc-200 outline-none focus:border-blue-500"
                value={nameValue}
                onChange={e => setNameValue(e.target.value)}
                onKeyDown={e => e.key === 'Enter' && handleSaveName()}
                autoFocus
              />
              <Button size="sm" className="text-xs h-6 px-2" onClick={handleSaveName}>Save</Button>
              <Button size="sm" variant="ghost" className="text-xs h-6 px-2" onClick={() => setEditingName(false)}>Cancel</Button>
            </div>
          ) : (
        ...
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-zinc-500 truncate flex-1">{rootPath || 'Not set'}</span>
            <button
              onClick={async () => {
        ...
        {filePath && (
          <div className="space-y-1">
            ...
            <span className="text-[10px] text-zinc-500 block truncate">{filePath}</span>
          </div>
        )}
      </div>
      ...
      {/* Jira Integration */}
      <div className="space-y-2">
        <label className="text-[11px] text-zinc-400 font-medium">Jira Connection</label>
        {jiraConnections.length > 0 ? (
          <select
            className="w-full bg-zinc-800 border border-zinc-600 rounded px-2 py-1 text-xs text-zinc-200 outline-none focus:border-blue-500"
          ...
          <span className="text-[10px] text-zinc-500 block">No Jira connections configured.</span>
        )}
        ...
        <div className="flex gap-1.5">
          <input
            className="flex-1 bg-zinc-800 border border-zinc-600 rounded px-2 py-1 text-xs text-zinc-200 outline-none focus:border-blue-500 placeholder-zinc-500"
            placeholder="Project key (e.g. COND)"
            value={spaceKeyInput}
            onChange={e => setSpaceKeyInput(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && addSpaceKey()}
          />
          <Button size="sm" variant="secondary" className="text-xs h-6 px-2" onClick={addSpaceKey}>
      ...
      <div className="space-y-2">
        <div>
          ...
          <div className="text-[10px] text-zinc-500">Mouse wheel for scrollback instead of arrow keys</div>
        </div>
        ...
        <div className="flex items-center justify-between bg-zinc-900/50 rounded px-2 py-1">
          <span className="text-[10px] text-zinc-400">Workspace{activeWorkspace ? ` (${activeWorkspace})` : ''}</span>
          <select
            className="bg-zinc-800 border border-zinc-600 rounded px-1.5 py-0.5 text-[10px] text-zinc-200 outline-none focus:border-blue-500"
